...
export class AvaliacaoData {
    async create(avaliacao: Avaliacao): Promise<Avaliacao> {
        const [newAvaliacao] = await dbConnection('avaliacoes').insert(avaliacao).returning('*');
        return newAvaliacao;
    }
    ...
    async update(id: number, avaliacao: Partial<Avaliacao>): Promise<Avaliacao | undefined> {
        const [updatedAvaliacao] = await dbConnection('avaliacoes').where({ id }).update(avaliacao).returning('*');
